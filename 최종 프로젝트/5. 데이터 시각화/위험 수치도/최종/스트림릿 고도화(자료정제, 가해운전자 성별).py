...
for name, group_df in group_dfs.items():
    ...
    vc = group_df['가해운전자 성별'].value_counts().reset_index()
    vc.columns = ['가해운전자 성별', 'Count']
...
with pd.ExcelWriter('동별 가해운전자 성별.xlsx') as writer:
    for name, df in group_dfs.items():
        vc = df['가해운전자 성별'].value_counts().reset_index()
        vc.columns = ['가해운전자 성별', 'Count']
        vc['동'] = name
