...
for name, group_df in group_dfs.items():
    ...
    vc = group_df['사고유형1'].value_counts().reset_index()
    vc.columns = ['사고유형1', 'Count']
...
for name, group_df in group_dfs2.items():
    # '값' 열에 대한 value_counts 계산
    vc = group_df['사고유형2'].value_counts().reset_index()
    vc.columns = ['사고유형2', 'Count']
...
with pd.ExcelWriter('동별 사고유형1.xlsx') as writer:
    for name, df in group_dfs.items():
        vc = df['사고유형1'].value_counts().reset_index()
        vc.columns = ['사고유형1', 'Count']
        vc['동'] = name
...
with pd.ExcelWriter('동별 사고유형2.xlsx') as writer:
    for name, df in group_dfs2.items():
        vc = df['사고유형2'].value_counts().reset_index()
        vc.columns = ['사고유형2', 'Count']
        vc['동'] = name
        vc.to_excel(writer, sheet_name=name, index=False)
